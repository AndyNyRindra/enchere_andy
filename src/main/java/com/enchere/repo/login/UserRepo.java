...
public interface UserRepo extends JpaRepository<User, Long> {
    ...
    @Query(value = "SELECT \"user\".* FROM enchere join \"user\" on \"user\".id=enchere.id_user GROUP BY id_user,\"user\".id order by count(enchere.id)  desc Limit 10 ", nativeQuery = true)
    public List<User> top10Populaire();

    @Query(value = "SELECT \"user\".* FROM enchere join \"user\" on \"user\".id=enchere.id_user GROUP BY id_user,\"user\".id order by sum(comission)  desc Limit 10 ", nativeQuery = true)
